...
// tweaks-panel.jsx — Reusable Tweaks shell + form-control helpers (verbatim from BLADE)
...
function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  React.useEffect(() => {
    ...
    return () => ro ? ro.disconnect() : window.removeEventListener('resize', clampToViewport);
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio" aria-checked={on}
                    data-on={on ? '1' : '0'} style={{ background: hero }} onClick={() => onChange(o)}>
